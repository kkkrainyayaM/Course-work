...
int Menu::registration() {
	...
	while (menu2 != 27) {
		cout << "РЕГИСТРАЦИЯ" << endl;
			...
			user->setID(IDGenerator::getInstance()->getUserId());
			user->readName();
			user->setLog();
			user->setPas();
			//DAO* daoInstance = DAO::getInstance();
			//daoInstance->saveUser(*participant);
			char* userbuf = reinterpret_cast<char*>(&user);
			send(ConnectSocket, userbuf, sizeof(User), 0);
